...
| 23 | `ember-scorch` | Burned wall scorch / soot stain | `wallFeatures` (F1 east walls) | **Shipped** — AI-generated 2026-08-12, replacing the C-graded centered-rectangle mark; uses the F1 wall tileset as a color palette (see prompt #23) |
...
### 23. Ember scorch mark (F1 wall decal, `wallFeatures`)

```text
ROLE: senior SNES-era JRPG pixel artist creating original dungeon wall decals.
GOAL: a single readable wall decal, a dark sooty scorch stain with a lighter ashy edge on the F1 Flooded Crypt wall, replacing one patch of wall texture.
STYLE: authentic hand-pixeled SNES 16-bit pixel art, hard pixel edges, no antialiasing/blur/vector/3D/photoreal, limited palette 8-12 opaque colors, controlled ramps from wall stone to light ashen soot to dark charred shadow, sparse single-pixel accents, straight-on wall-face view only, not a scene or portrait, implied top-left light, dark fantasy damp mood.
COMPOSITION: one single scorch only, the scorch is a wide low jagged stain in the bottom one-third of a 64x64 canvas, touching the bottom edge, the rest of the canvas is the normal F1 wall, not darkened, no floor/characters/UI/text/watermarks.
BACKGROUND: flat opaque wall-stone background filling the entire canvas, the F1 Flooded Crypt wall texture, mossy olive-grey coursed ashlar stone, not brick, not tile, not concrete, this is a wall decal not a transparent sprite, no chroma key, no dark vignette.
NEGATIVE: photoreal, soft gradients, oil painting, 3D, modern HD pixel, glossy highlights, neon, cluttered diorama, readable runes, logos, watermark, multiple objects, multiple scorches, two scorches, two patches, centered neat rectangle or oval, frame, plaque, metal plate, torch, sconce, brazier, candle, lamp, flame, fire, flicker, burning object, lit wick, vertical tongue, teardrop shape, ember, ember glow, glowing embers, red, orange, yellow, warm colors, bright red, bright orange, bright yellow, any large warm mass, glow, bloom, blood, wound, hand, skull, face, eyes, robot or helmet pareidolia, green moss over the scorch, brick pattern, red brick, uniform rectangular blocks, transparent or chroma background, floating object, dark frame, dark border, alcove, niche, recess, holder, bracket, colored border around the scorch, circle, round shape, oval shape, perfect circle, smooth dome top, arch, doorway, portal, circular opening, solid black silhouette with no internal texture, dark vignette over the whole canvas, broad shadow over the whole canvas, symmetrical shape, mirrored halves.
SUBJECT: a single dark jagged scorch stain on the F1 Flooded Crypt dungeon wall, viewed dead-on as a wall-mounted decal. The scorch is one irregular jagged-edged patch of blackened/charred stone in the bottom one-third of the canvas, touching the bottom edge, wider at the base and tapering upward with asymmetric soot tendrils, not smooth or rounded, not a geometric shape, not a circle, not an oval, not an arch, not a face. The scorch is lopsided and unsymmetrical, with one side wider and higher than the other, the top edge has irregular peaks and notches, the sides are uneven, the bottom is jagged. The scorch is clearly darker and distinct from the surrounding F1 wall, with a visible mottled texture. It has a slightly lighter, dusty ashen edge 2-4 pixels wide where soot thins, giving the shape a visible rim, then the interior is black and charcoal. There is no flame and no burning object. The scorch is a flat discoloration on the wall, not a 3D object attached to it. The scorched patch has internal texture: lighter soot pixels, dark grey and charcoal cracks, small variations, not a solid flat black shape. Surface: the scorch center is deep charcoal with tiny dark grey and umber crack lines and a few dull warm umber flecks. Materials: the wall is mossy olive-grey damp ashlar stone, not brick, the scorch is matte black soot, not glossy, umber and dark grey only in tiny internal cracks. No red, no orange, no yellow. Must not have: any flame, any torch, any fire, any lit object, multiple scorches, a centered circle or oval, frame, plaque, metal plate, dark frame, colored border, bright red or orange or yellow mass, blood, wound, hand, skull, face, eyes, symmetric dark cavities, robot or helmet pareidolia, green moss, brick pattern, uniform blocks, transparent background, floating object, solid flat black shape, vignette, broad shadow, circular opening, arch.
```

**Shipped 2026-08-12 after ten candidates.** The original `ember-scorch` was C-graded for reading as another centered rectangle on the F1 wall. The first generation pass produced a wall-mounted torch/brazier because "scorch" kept being read as a lit flame. Adding "no torch, no flame, no red/orange/yellow" kept the candidates flame-free but the model repeatedly used any available rust color as a bright outline, producing a red-ringed circle/portal or a small red ember blob. The accepted candidate used the F1 wall tileset (`public/assets/tilesets/f1/wall.png`) as a `--palette` to lock the wall color, then removed all warm colors from the positive list and asked for a darker, low, irregular sooty scorch with a slightly lighter ashen edge for contrast. It is a `wallFeatures` opaque decal (generated with `--background` and `--size 64x64`), bottom-anchored, and is verified at F1 (26,6) east via `scripts/playtests/verify-ember-scorch.mjs` with zero console/page errors. The warm ember accent from the name is deliberately subtle to avoid torch/ember misreadings; the result is a black/charcoal scorch on F1 stone.
