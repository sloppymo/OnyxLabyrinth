...
1. **NPC maze-sprite hook** — Tier C, single small addition (`NPCDef.mapSpriteId?` + billboard draw at the NPC's tile), pays off for all 5 NPCs at once (Oren, Rill-of-Pages, Tallow-in-a-Boat, Sister Caldris, Vestra, Kazeharu, Vesper, Ossian — 8 total across the campaign). Currently every one of them is a bare `&` glyph. Highest leverage-per-line-of-code item in this audit.
