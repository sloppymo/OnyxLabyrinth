...
- Cards have an explicit owner: `old-man` or `rat-king`. There is no player-facing school selection, neutral pool, Open/Bound/Regalia legality, or cross-deck movement in the first campaign slice. Old Man build selection (below) is not school selection: it only picks the *starting* contents of his one collection, which then evolves through discovery exactly as described here.
...
## Canonical Old Man build selection

**Added 2026-08-29. This is a deliberate, narrow reversal of the earlier blanket "no character creation" and "no player-facing school selection" language, for Old Man only.**

New Game presents exactly three authored Old Man starting decks before any campaign state exists. The player picks one; it fixes only the *starting* contents of Old Man's one collection/active deck. Everything downstream is unchanged: one evolving collection, one active deck, authored place-bound discoveries, exact 12-card decks, the two-copy cap. Choosing a build is not selecting a school, is not creating a character, and does not gate which cards Old Man can later discover.

- The three builds are **The Silent Ward** (Hush/control), **The Last Hour** (Opened/Omen), and **The Reckoning** (Opened-consuming burst). Each is an exact 12-card starting deck built from eight unique definitions, two universal Old Man basics (`distant-hand`, `pale-ward`) plus build-specific cards, including a small set of build-exclusive signature cards (`veil-of-quiet`, `the-quiet-after`, `silence-the-hall`, `hasten-the-hour`, `the-final-word`, `reckoning-strike`, `brace-for-it`) that use only the already-live card vocabulary — Hush, Opened, Barrier, Omen, Energy, Move. No new state noun was added for this feature.
...
- A save records which build Old Man started with (`CampaignCardProgress.oldManBuildId`). A save written before this feature existed has no such field and is treated as build id `"legacy"` — the exact generalist starter deck Old Man always had — so no existing save's deck is ever silently rewritten.
- Rat King has no build choice. He keeps his single fixed starter deck, unchanged.
...
- Full design rationale, all three decks' exact card text, and the dominance/balance checks performed before shipping are in [`design/2026-08-29-old-man-build-selection.md`](design/2026-08-29-old-man-build-selection.md).

## Canonical campaign

- The protagonists are the fixed duo **Old Man** and **Rat King**.
- There is no preset party, selectable roster, Reform Party, formation administration, or playable companion system. **Exception (added 2026-08-29):** New Game lets the player choose one of three authored Old Man starting decks/playstyles — see "Canonical Old Man build selection" below. This is a one-time starting-collection choice for the fixed protagonist, not character creation, a roster, or a companion system. Rat King has no build choice and keeps his single fixed starter.
- New Game follows **Title → Old Man build selection → Prologue → Labyrinth entrance → Kept Gate → abyss bridge and Face → first refuge → wider exploration**. Build selection runs once, before any campaign state exists; canceling out of it returns to Title with no side effect.
