...
3. **`encounterTable` is dead.** The field is deprecated and ignored. Combat encounter tables live in `ENCOUNTER_TABLES` in `src/data/enemies.ts`, keyed by floor id (**1–5** for the campaign) or a zone's `tableFloorId`. A custom floor id with no table gets **no random encounters** — unless you paint encounter zones with `tableFloorId` pointing at an existing table (this is what the demo floor does). Packs cannot yet define their own tables. **Campaign climax chests use `tableFloorId`:** F2 forbidden-wing → 6, F3 Grand Forge → 7, F4 sanctum → 8, F5 undersong → 9. Other safe/hot zones share that floor's ambient table (frequency differs; pack difficulty does not). Default pity forces a fight by step 28 even when `rateMul` is 0; Floor 1 authors a slower `encounterPacing` profile.
