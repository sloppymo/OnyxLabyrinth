...
- **Two perk mechanisms, pick whichever fits a given perk's effect shape:**
  - **`perkModifiers(perks, effStats)`** (`game/perks.ts`) — for simple always-on numeric passives (damage/spell/crit/evasion/flee multipliers and bonuses, physical damage-taken multipliers with an optional front-row-only bucket, SP cost multipliers, flat bonus damage, trap disarm/damage modifiers). Folds every chosen perk into one struct; combat/features/town code reads it fresh each time, never caches it. This covers most of the 56 perks with zero combat-internals changes. Caveat: `shopDiscountPercent` is aggregated but NOT yet consumed by `town-ui.ts` — Swindler's discount is inert (`TODO(v1.1)`).
  - **`dispatchHook(hook, perks, ctx)`** (`game/perks.ts`) — for stateful/reactive perks (once-per-combat triggers, counterattacks, escalating stacks, "every Nth cast"). `CombatState.perkState` (character id → arbitrary bag) holds each character's per-combat scratch data, reset at combat start. ~19 perks have a registered handler (see the list at the top of `game/perks.ts`); a few more are wired directly in `combat.ts` (fighter-protector, thief-backstab, thief-assassin, duelist-riposte, priest-saint's regen, mage-spellbreaker, halberdier-warlord); the rest are numeric-passive or documented `// TODO(v1.1)` stubs in `data/perks.ts` whose UI descriptions carry a "(Not yet implemented — v1.1.)" marker.
