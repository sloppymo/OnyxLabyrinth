...
- **`__onyxDebug.isIdle()`**: true when nothing is animating that a script should wait out — no pending mode fade (`transitionToMode`'s 150ms opacity swap, tracked by `modeTransitionPending`), no render-camera tween (`isRenderCameraAnimating()`), no prologue auto-play, and no unfinished combat playback choreography. Menus, camp, town, and combat's non-playback phases (palette/select*/result) all count as idle-awaiting-input on purpose — this is "is it safe to read state / press the next key," not "is the player doing nothing." The decision logic is the pure, exhaustively-unit-tested `computeIdle()` in `src/debug/idle.ts` (`src/debug/idle.test.ts`); `main.ts`'s `isIdle()` is a thin call with live values, and `CombatController.isChoreographyDone()` is the cheap (no array-copy) combat seam feeding it — don't call `debugView()` from a polling loop, it allocates enemy/log arrays every tick.
