...
- **Two perk mechanisms, pick whichever fits a given perk's effect shape:**
  ...
  - **`dispatchHook(hook, perks, ctx)`** (`game/perks.ts`) — for stateful/reactive perks (once-per-combat triggers, counterattacks, escalating stacks, "every Nth cast"). `CombatState.perkState` (character id → arbitrary bag) holds each character's per-combat scratch data, reset at combat start. ~16 perks have a registered handler (see the list at the top of `game/perks.ts`); a few more are wired directly in `combat.ts` (fighter-protector, thief-backstab, thief-assassin, duelist-riposte, priest-saint's regen); the rest are numeric-passive or documented `// TODO(v1.1)` stubs in `data/perks.ts` whose UI descriptions carry a "(Not yet implemented — v1.1.)" marker.
