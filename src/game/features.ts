 ...
 */
export function disarmChest(state: GameState, rng: Rng = getGameplayRng()): ChestActionResult {
  const p = state.pendingTrap;
...
export function openChest(state: GameState, rng: Rng = getGameplayRng()): ChestActionResult {
  const p = state.pendingTrap;
  const treasure = pendingTreasure(state);
  if (!p || !treasure) return noChest();
