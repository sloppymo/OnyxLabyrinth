...
describe("Floor 1 revision", () => {
  it("floor 1 has floorRevision at or above 3 (bumped again when Hot Boi's tavern moved to its own room)", () => {
    const floor = findFloor(1)!;
    expect(floor.floorRevision).toBeGreaterThanOrEqual(3);
  });
});
