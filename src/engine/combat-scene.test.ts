...
describe("playTurn choreography", () => {
  it("melee attack schedules approach, impact popup, and return", () => {
    ...
    updateScene(scene, t0 + 600); // past IMPACT_AT (~568)
  ...
  it("cast shows the spell banner and bursts on targets", () => {
    const scene = makeScene();
    const events: CombatEvent[] = [
      { type: "cast", actorId: "c1", spellId: "mage-halito", targetId: "rat-0" },
      { type: "spellEffect", spellId: "mage-halito", targetId: "rat-0", damage: 5 },
    ];
    const t0 = 0;
    playTurn(scene, events, spellName, t0, W, H);
    updateScene(scene, t0 + 10);
    expect(scene.banner).toBe("Spell:mage-halito");
